...
async def execution_welcome(message: Message, first_name: str):
    try:
        ...
        if custom_welcome:
            welcome_text = custom_welcome.replace("{name}", first_name).replace("{first_name}", first_name)
        else:
            welcome_text = (
                f"Hey {first_name} 👋\n"
...
# --- Admin: send a file directly to the bot to auto-store + generate a link ---
@router.message(F.document | F.video | F.audio | F.photo | F.voice | F.animation)
async def handle_direct_upload(message: Message):
    ...
    if message.document:
        ...
        name = message.audio.file_name or message.audio.title or "Audio"
    elif message.photo:
        name = "Photo"
    elif message.voice:
        name = "Voice Message"
    elif message.animation:
        name = message.animation.file_name or "Animation"
    ...
    try:
        ...
        LOG.error(f"Failed to copy uploaded file to storage channel: {e}")
        await message.answer(
            "❌ <i>Failed to store this file — make sure the bot is an admin in the storage channel.</i>"
        )
        return
    ...
    if message.from_user.id in BATCH_SESSIONS:
        ...
        await message.answer(
            f"✅ <b>Added to batch</b> (<code>{count}</code> file{'s' if count != 1 else ''} so far).\n"
            f"<i>Send more files, or</i> <code>/done</code> <i>to finish.</i>"
        )
        return

    # Single file — generate its link immediately
...
async def set_welcome_handler(message: Message, command: CommandObject):
    ...
    if not text:
        await message.answer(
            ...
            "<i>Use</i> <code>{name}</code> <i>anywhere to insert the user's first name.</i>\n"
    ...
    await message.answer(text.replace("{name}", message.from_user.first_name or "User"))
