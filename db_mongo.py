...
class MongoDatabase:
    ...
    async def init(self) -> None:
        ...
        try:
            ...
            LOG.info(f"✅ MongoDB connected to database '{self._db_name}'")
            ...
            raise RuntimeError(
                ...
                "2. MongoDB server is running and accessible\n"
    ...
    async def restore_backup(self, backup: dict) -> dict:
        ...
        SAFETY: This method never deletes or modifies existing documents.
        It only inserts new documents that don't already exist (by unique key).
        If a document with the same key exists, it's skipped — the restore
        cannot overwrite it."""
        error = self._validate_backup(backup)
        if error:
            raise ValueError(f"Backup validation failed: {error}")

        LOG.info(f"🔄 Starting restore from backup (schema v{backup.get('schema_version')}, {len(backup.get('tables', {}))} collections)")
        ...
        for table, rows in backup.get("tables", {}).items():
            ...
            for row in rows:
                try:
                    # Identify unique key: user_id, token, code, id, or key field
                    key = {k: row[k] for k in row if k in ("user_id", "token", "code", "id", "key")} or {"_id": row.get("_id")}
            ...
            LOG.debug(f"  {table}: {inserted} inserted, {skipped_count} skipped (existing), {failed_count} errors")
        
        LOG.info(f"✅ Restore complete: {sum(report.values())} rows inserted, {skipped_count} skipped, {failed_count} errors")
        return report
