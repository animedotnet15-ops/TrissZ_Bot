...
async def backup_handler(message: Message):
    ...
    await message.answer_document(
        ...
        caption=(
            ...
            f"🗃 <b>Backend:</b> <code>{backup.get('backend', config.db_backend)}</code>\n"
            f"🏷 <b>Schema version:</b> <code>{backup.get('schema_version')}</code>\n\n"
...
async def restore_handler(message: Message):
    ...
    if validation_error:
        await message.answer(f"❌ <b>Backup failed validation:</b> <code>{validation_error}</code>\n\n<i>Nothing was touched.</i>")
        return
    ...
    try:
        ...
        await message.answer_document(
            ...
            caption="🛡 <b>Automatic safety backup</b> (taken right before this restore ran).",
        )
    except Exception as e:
    ...
    try:
        ...
        await status.edit_text(
            f"❌ <b>Restore failed and was rolled back:</b> <code>{e}</code>\n\n"
    ...
    if hasattr(database, "verify_backup_integrity"):
        try:
            ...
            if not verify_ok:
                verify_note = f"\n\n⚠️ <b>Integrity check found orphaned rows:</b> <code>{integrity['orphans']}</code>"
        except Exception as e:
...
async def fsub_panel_handler(message: Message):
    ...
    await message.answer(
        ...
        f"📊 <b>Configured:</b> <code>{len(channels)}</code>\n"
...
async def fsub_callback_handler(cb: CallbackQuery):
    ...
    if action == "remove":
        ...
        try:
            await cb.message.edit_text(
                f"⚠️ <b>Remove Force-Subscribe channel</b> <code>{label}</code><b>?</b>",
...
async def _handle_pending_fsub_input(message: Message, pending: dict) -> bool:
    ...
    if pending["action"] == "fsub_edit":
        ...
        channels = await database.get_fsub_channels()
        await message.answer(
            "✅ <b>Label updated.</b>" if ok else "⚠️ <i>That entry no longer exists.</i>",
            reply_markup=fsub_panel_keyboard(channels),
        )
        return True
...
async def button_manager_handler(message: Message):
    ...
    await message.answer(
        "🔘 <b>Button Manager</b>\n\n"
        f"📊 <b>Configured:</b> <code>{len(buttons)}</code>\n"
...
async def button_manager_callback_handler(cb: CallbackQuery):
    ...
    if action == "delete":
        ...
        try:
            await cb.message.edit_text(
                f"⚠️ <b>Delete button</b> <code>{btn['text']}</code><b>?</b>",
...
async def _handle_pending_button_input(message: Message, pending: dict) -> bool:
    ...
    if pending["action"] == "button_add_target":
        ...
        buttons = await database.list_buttons()
        await message.answer(
            f"✅ <b>Button added</b> (<code>#{btn_id}</code>).",
            reply_markup=button_manager_keyboard(buttons),
        )
        return True
    ...
    if pending["action"] == "button_edit_text":
        ...
        buttons = await database.list_buttons()
        await message.answer(
            "✅ <b>Button updated.</b>" if ok else "⚠️ <i>That button no longer exists.</i>",
            reply_markup=button_manager_keyboard(buttons),
        )
        return True
...
def _welcome_panel_text(cfg: dict) -> str:
    return (
        "🎨 <b>Welcome Customization</b>\n\n"
        f"📝 <b>Text:</b> <code>{'custom' if cfg.get('text') else 'default'}</code>\n"
        f"🖼 <b>Photo:</b> <code>{'set' if cfg.get('photo_id') else 'none'}</code>\n"
        f"🎟 <b>Sticker:</b> <code>{'set' if cfg.get('sticker_id') else 'none'}</code>\n"
        f"🔌 <b>Status:</b> <code>{'ENABLED' if cfg.get('enabled') else 'DISABLED'}</code>"
    )
...
async def welcome_callback_handler(cb: CallbackQuery):
    ...
    if action == "edit_text":
        ...
        await cb.message.answer(
            "✏️ <b>Send the new welcome message</b> (formatting carries over).\n\n"
    ...
    if action == "set_photo":
        ...
        await cb.message.answer("🖼 <b>Send the photo now.</b>")
        await cb.answer()
        return

    if action == "set_sticker":
        PENDING_ADMIN_INPUT[cb.from_user.id] = {"action": "welcome_set_sticker"}
        await cb.message.answer("🎟 <b>Send the sticker now.</b>")
        await cb.answer()
        return
    ...
    if action == "reset":
        try:
            await cb.message.edit_text(
                "⚠️ <b>Reset ALL welcome customization to default?</b>\n"
...
async def _handle_pending_welcome_input(message: Message, pending: dict) -> bool:
    ...
    if action == "welcome_edit_text":
        ...
        del PENDING_ADMIN_INPUT[admin_id]
        cfg = await database.get_welcome_config()
        await message.answer("✅ <b>Welcome text updated!</b>", reply_markup=welcome_panel_keyboard(cfg))
        return True
    ...
    if action == "welcome_set_photo":
        ...
        del PENDING_ADMIN_INPUT[admin_id]
        cfg = await database.get_welcome_config()
        await message.answer("✅ <b>Welcome photo updated!</b>", reply_markup=welcome_panel_keyboard(cfg))
        return True
    ...
    if action == "welcome_set_sticker":
        ...
        del PENDING_ADMIN_INPUT[admin_id]
        cfg = await database.get_welcome_config()
        await message.answer("✅ <b>Welcome sticker updated!</b>", reply_markup=welcome_panel_keyboard(cfg))
        return True
...
async def finishbatch_handler(message: Message):
    ...
    await message.answer(
        ...
        f"📊 <b>Files:</b> <code>{len(file_ids)}</code>\n"
        f"🆔 <b>First message ID:</b> <code>{active['first_message_id']}</code>\n"
        f"🆔 <b>Last message ID:</b> <code>{message.message_id}</code>\n"
        f"📥 <b>Link:</b> <code>{share_url}</code>"
    )
...
def _batch_detail_text(batch: dict) -> str:
    lines = [
        ...
        f"📊 <b>Status:</b> <code>{batch.get('status')}</code>",
        f"🗂 <b>Files:</b> <code>{len(batch.get('files', []))}</code>",
    ]
    if batch.get("first_message_id"):
        lines.append(f"🆔 <b>First message:</b> <code>{batch['first_message_id']}</code>")
    if batch.get("last_message_id"):
        lines.append(f"🆔 <b>Last message:</b> <code>{batch['last_message_id']}</code>")
    if batch.get("post_code"):
        lines.append(f"📥 <b>Link:</b> <code>https://t.me/{config.bot_username}?start=file_{batch['post_code']}</code>")
    return "\n".join(lines)
