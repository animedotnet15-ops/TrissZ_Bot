...
async def process_start_payload(message: Message, user, payload: str):
    ...
    if int(post["protected"]):
        ...
        # Wrap our own guard webpage (not a raw t.me deep link) in the shortener,
        # so Arolinks sees a real page load and actually credits the wallet.
        guard_page_url = f"{config.base_url}/g/{code}?tok={token}"
        verification_link = await shorten_with_arolinks(guard_page_url)
