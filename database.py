...
class Database:
        ...
        async with self._init_lock:
            ...
            async with self.connection() as db:
                await db.executescript(
                    ...
                    -- Runtime-managed admins/moderators, ON TOP OF the
                    -- permanent .env ADMIN_IDS (which always keep full
                    -- access - this table never overrides or removes
                    -- those, only adds more people at the OWNER's
                    -- discretion, matching "never break existing
                    -- functionality").
                ...
                # --- Safe migration: Link Manager analytics columns on `posts` ---
                post_columns = {
                    row[1]
                    for row in await (
                        await db.execute("PRAGMA table_info(posts)")
                    ).fetchall()
                }
                link_columns = {
                ...
                for col_name, col_def in link_columns.items():
                    if col_name not in post_columns:
                        await db.execute(f"ALTER TABLE posts ADD COLUMN {col_name} {col_def}")
                await db.execute(
    ...
    # ------------------------------------------------------------------ #
    #  Admin roles (runtime-managed, ON TOP OF the permanent .env admins)
    # ------------------------------------------------------------------ #
    async def add_admin_role(self, user_id: int, role: str, added_by: int) -> None:
        role = role.upper()
        ...
        async with self.connection() as db:
            await db.execute(
                """
                INSERT INTO admin_roles(user_id, role, added_by, added_at, enabled)
                VALUES(?, ?, ?, ?, 1)
                ON CONFLICT(user_id) DO UPDATE SET
                    role = excluded.role, enabled = 1
                """,
                (user_id, role, added_by, int(time.time())),
            )
            await db.commit()

    async def remove_admin_role(self, user_id: int) -> bool:
        async with self.connection() as db:
            cur = await db.execute("DELETE FROM admin_roles WHERE user_id = ?", (user_id,))
            await db.commit()
            return cur.rowcount > 0

    async def get_admin_role(self, user_id: int) -> str | None:
        """Returns 'ADMIN' or 'MODERATOR' for runtime-added admins, or
        None if the user has no DB-tracked role (they may still be a
        permanent .env admin/owner - that's checked separately in bot.py,
        since it doesn't need the database at all)."""
        async with self.connection() as db:
            row = await (
                await db.execute(
                    "SELECT role FROM admin_roles WHERE user_id = ? AND enabled = 1", (user_id,)
                )
            ).fetchone()
            return row["role"] if row else None

    async def list_admin_roles(self) -> list[aiosqlite.Row]:
        async with self.connection() as db:
            return await (
                await db.execute("SELECT * FROM admin_roles WHERE enabled = 1 ORDER BY added_at ASC")
            ).fetchall()
    ...
    async def get_dashboard_stats(self) -> dict:
        now = int(time.time())
        online_cutoff = now - 300           # last 5 minutes
        today_cutoff = now - 86400
        yesterday_start = now - 2 * 86400
        yesterday_end = today_cutoff
        ...
        async with self.connection() as db:
            ...
            online_users = await _count(
                "SELECT COUNT(*) FROM users WHERE last_seen >= ?", (online_cutoff,)
            )
            today_users = await _count(
                "SELECT COUNT(*) FROM users WHERE created_at >= ?", (today_cutoff,)
            )
            yesterday_users = await _count(
                "SELECT COUNT(*) FROM users WHERE created_at >= ? AND created_at < ?",
                (yesterday_start, yesterday_end),
            )
            weekly_users = await _count(
                "SELECT COUNT(*) FROM users WHERE created_at >= ?", (week_cutoff,)
            )
            monthly_users = await _count(
                "SELECT COUNT(*) FROM users WHERE created_at >= ?", (month_cutoff,)
            )
            banned_users = await _count("SELECT COUNT(*) FROM bans")
            total_files = await _count("SELECT COUNT(*) FROM stored_files")
            total_links = await _count("SELECT COUNT(*) FROM posts")
            total_verifications = await _count("SELECT COALESCE(SUM(verification_count), 0) FROM users")
            total_downloads = await _count("SELECT COALESCE(SUM(download_count), 0) FROM users")
    ...
    # ------------------------------------------------------------------ #
    #  Link Manager: analytics, revoke, regenerate, search, pagination
    # ------------------------------------------------------------------ #
    async def _increment_post_field(self, code: str, field: str) -> None:
        async with self.connection() as db:
            await db.execute(
                f"UPDATE posts SET {field} = {field} + 1 WHERE code=?", (code,)
            )
            await db.commit()

    async def increment_link_click(self, code: str) -> None:
        await self._increment_post_field(code, "click_count")

    async def increment_link_verify_success(self, code: str) -> None:
        await self._increment_post_field(code, "verify_success_count")

    async def increment_link_verify_failed(self, code: str) -> None:
        await self._increment_post_field(code, "verify_failed_count")

    async def increment_link_delivery(self, code: str) -> None:
        await self._increment_post_field(code, "delivery_count")

    async def increment_link_delivery_failed(self, code: str) -> None:
        await self._increment_post_field(code, "delivery_failed_count")

    async def is_link_active(self, code: str) -> bool:
        async with self.connection() as db:
            row = await (
                await db.execute("SELECT status FROM posts WHERE code=?", (code,))
            ).fetchone()
        return bool(row) and row["status"] == "active"

    async def revoke_link(self, code: str) -> bool:
        async with self.connection() as db:
            cur = await db.execute(
                "UPDATE posts SET status='revoked', revoked_at=? WHERE code=? AND status='active'",
                (int(time.time()), code),
            )
            await db.commit()
            return cur.rowcount > 0

    async def reactivate_link(self, code: str) -> bool:
        async with self.connection() as db:
            cur = await db.execute(
                "UPDATE posts SET status='active', revoked_at=NULL WHERE code=? AND status='revoked'",
                (code,),
            )
            await db.commit()
            return cur.rowcount > 0

    async def regenerate_link(self, old_code: str) -> str | None:
        """Revokes the old code and creates a fresh code pointing at the same
        files/protection — old links stop working immediately."""
        old_post = await self.get_post(old_code)
        if not old_post:
            return None
        file_rows = await self.get_post_files(int(old_post["id"]))
        file_ids = [int(f["id"]) for f in file_rows]
        if not file_ids:
            return None

        await self.revoke_link(old_code)
        new_row = await self.create_post(
            kind=old_post["kind"], file_ids=file_ids, protected=bool(old_post["protected"])
        )
        return new_row["code"]

    async def get_link_analytics(self, code: str) -> dict | None:
        post = await self.get_post(code)
        if not post:
            return None
        file_count = len(await self.get_post_files(int(post["id"])))
        return {
            "code": post["code"],
            "kind": post["kind"],
            "protected": bool(post["protected"]),
            "status": post["status"],
            "created_at": post["created_at"],
            "revoked_at": post["revoked_at"],
            "file_count": file_count,
            "click_count": post["click_count"],
            "verify_success_count": post["verify_success_count"],
            "verify_failed_count": post["verify_failed_count"],
            "delivery_count": post["delivery_count"],
            "delivery_failed_count": post["delivery_failed_count"],
        }

    async def search_links(self, query: str, limit: int = 20) -> list[aiosqlite.Row]:
        like = f"%{query.strip()}%"
        async with self.connection() as db:
            return await (
                await db.execute(
                    "SELECT * FROM posts WHERE code LIKE ? ORDER BY created_at DESC LIMIT ?",
                    (like, limit),
                )
            ).fetchall()

    async def list_links_page(self, page: int, page_size: int = 10) -> tuple[list[aiosqlite.Row], int]:
        """Returns (rows_for_page, total_count) newest-first."""
    ...
    async def get_post_code_for_token(self, token: str) -> str | None:
        """Read-only lookup used purely for link analytics attribution -
        never mutates state, safe to call alongside claim_token."""
        async with self.connection() as db:
            row = await (
                await db.execute(
                    """
                    SELECT p.code FROM pending_tokens t
