...
class Database:
    ...
    async def init(self) -> None:
        async with self._init_lock:
            ...
            async with self.connection() as db:
                ...
                # --- Safe migration: link revocation on `posts` (Backup/Genlink) ---
                post_columns = {
                    row[1]
                    for row in await (await db.execute("PRAGMA table_info(posts)")).fetchall()
                }
                if "revoked" not in post_columns:
                    await db.execute("ALTER TABLE posts ADD COLUMN revoked INTEGER NOT NULL DEFAULT 0")

                # --- Safe migration: message-range fields on `custom_batches`
                # (FEATURE 2 — First Message -> Last Message Custom Batch).
                # Reuses the existing custom_batches/custom_batch_files tables
                # instead of creating a second batch architecture. Old rows
                # (created by the legacy name/description editor, if any)
                # default to status='completed' so they aren't mistaken for
                # an in-progress collection session. ---
                cb_columns = {
                    row[1]
                    for row in await (await db.execute("PRAGMA table_info(custom_batches)")).fetchall()
                }
                cb_migrations = {
                ...
                for col_name, col_def in cb_migrations.items():
                    if col_name not in cb_columns:
                        await db.execute(f"ALTER TABLE custom_batches ADD COLUMN {col_name} {col_def}")

                # --- Safe migration: extra Welcome Customization fields.
                # text/photo_id/sticker_id/spoiler continue to live in
                # `settings` (unchanged, preserves existing production data);
                # this table now holds only the additional toggles the new
                # Welcome Customization UI needs, so the table stops being
                # dead/unused and nothing is duplicated. ---
                wc_columns = {
                    row[1]
                    for row in await (await db.execute("PRAGMA table_info(welcome_config)")).fetchall()
                }
                wc_migrations = {
                    "enabled": "INTEGER NOT NULL DEFAULT 1",
                    "sticker_anim_enabled": "INTEGER NOT NULL DEFAULT 0",
                }
                for col_name, col_def in wc_migrations.items():
                    if col_name not in wc_columns:
                        await db.execute(f"ALTER TABLE welcome_config ADD COLUMN {col_name} {col_def}")
    ...
    async def get_fsub_channels(self) -> list[dict]:
        ...
        if not isinstance(data, list):
            return []
        # Self-migrating defaults: channels added before the Force-Subscribe
        # Manager UI existed won't have "enabled"/"folder" keys yet — treat
        # them as enabled with no folder, same as their previous behavior.
        for ch in data:
    ...
    async def get_enabled_fsub_channels(self) -> list[dict]:
        """Only channels the admin has left enabled are ever checked/shown —
        used by the real membership-check flow."""
        return [c for c in await self.get_fsub_channels() if c.get("enabled", True)]
    ...
    async def update_fsub_channel(self, index: int, **kwargs) -> bool:
        """Edit folder/label (or any other field) of an existing entry."""
        channels = await self.get_fsub_channels()
        if not (0 <= index < len(channels)):
            return False
    ...
    async def get_post(self, code: str) -> aiosqlite.Row | None:
        ...
        if row:
            # Revoked links exist but are never resolved/delivered.
            return None if int(row["revoked"] or 0) else row
    ...
    async def revoke_post(self, code: str) -> bool:
        async with self.connection() as db:
            cur = await db.execute("UPDATE posts SET revoked=1 WHERE code=?", (code,))
            await db.commit()
            if cur.rowcount > 0:
                mongo_db = await self._get_mongo()
                if mongo_db is not None:
                    try:
                        await mongo_db.posts.update_one({"code": code}, {"$set": {"revoked": True}})
                    except Exception as e:
                        LOG.warning(f"Mongo mirror revoke failed: {e}")
                return True
            return False
    ...
    # ------------------------------------------------------------------ #
    # Every persistent SQLite table currently used by the repository.
    # Kept as one explicit list (rather than introspecting sqlite_master)
    # so a stray/temp table can never accidentally get swept into a
    # backup — but every table created in init() above IS listed here.
    _BACKUP_TABLES = (
        ...
        "scheduled_deletions", "link_analytics",
        "custom_batches", "custom_batch_files",
        "caption_templates", "thumbnail_configs",
        "button_configs", "welcome_config", "broadcasts",
    )
    # Table -> its foreign-key parent table (for post-restore relationship
    # verification: every child row's parent id must actually exist).
    _BACKUP_RELATIONSHIPS = {
        "post_files": ("post_id", "posts", "id"),
        "protected_sessions": ("post_id", "posts", "id"),
        "pending_tokens": ("post_id", "posts", "id"),
        "custom_batch_files": ("batch_id", "custom_batches", "id"),
        "link_analytics": ("post_id", "posts", "id"),
    }
    ...
    def _validate_backup(self, backup: dict) -> str | None:
        """Returns an error message, or None if the backup looks structurally sound."""
    ...
    async def restore_backup(self, backup: dict) -> dict:
        """Insert-only restore. Existing rows are NEVER touched, overwritten,
        or deleted — only rows missing from the current database are
        inserted. Runs inside a single transaction: if anything raises
        before the final commit, aiosqlite rolls the whole transaction
        back automatically (nothing partially applied)."""
    ...
    async def verify_backup_integrity(self) -> dict:
        """Post-restore verification: current row counts per table, plus a
        relationship check (every child row's foreign key actually resolves
        to an existing parent row). Used by /restore to confirm the
        database is consistent after restoring, and to decide whether an
        automatic rollback (re-restoring the pre-restore safety backup) is
        needed."""
        counts = {}
        orphans = {}
        async with self.connection() as db:
            for table in self._BACKUP_TABLES:
                row = await (await db.execute(f"SELECT COUNT(*) c FROM {table}")).fetchone()
                counts[table] = int(row["c"])
            for child, (fk_col, parent, parent_col) in self._BACKUP_RELATIONSHIPS.items():
                row = await (await db.execute(
                    f"SELECT COUNT(*) c FROM {child} c1 "
                    f"WHERE {fk_col} IS NOT NULL AND NOT EXISTS "
                    f"(SELECT 1 FROM {parent} p WHERE p.{parent_col} = c1.{fk_col})"
                )).fetchone()
                orphans[child] = int(row["c"])
    ...
    async def add_file_to_custom_batch(self, batch_id: int, file_id: int) -> bool:
        async with self.connection() as db:
            ...
            pos = (pos_row["p"] + 1) if pos_row["p"] is not None else 0
            await db.execute(
    ...
    # ------------------------------------------------------------------ #
    #  FEATURE 2: First Message -> Last Message Custom Batch.
    #  Reuses the custom_batches / custom_batch_files tables above (no
    #  duplicate batch architecture) with the message-range columns added
    #  in init(). Status lives in the DB (not memory), so an in-progress
    #  collection session survives a bot restart — resume by looking up
    #  the admin's active batch again instead of an in-memory dict.
    # ------------------------------------------------------------------ #
    async def start_message_batch(self, created_by: int, first_message_id: int) -> int:
        async with self.connection() as db:
            cur = await db.execute(
                "INSERT INTO custom_batches"
                "(name, created_by, created_at, first_message_id, status) "
                "VALUES(?, ?, ?, ?, 'collecting')",
                (f"batch-{int(time.time())}", created_by, int(time.time()), first_message_id),
            )
            await db.commit()
            return int(cur.lastrowid)
    ...
    async def get_active_message_batch(self, created_by: int) -> dict | None:
        async with self.connection() as db:
            row = await (await db.execute(
                "SELECT * FROM custom_batches WHERE created_by=? AND status='collecting' "
    ...
    async def finish_message_batch(self, batch_id: int, last_message_id: int) -> bool:
        async with self.connection() as db:
            cur = await db.execute(
                "UPDATE custom_batches SET status='completed', last_message_id=?, completed_at=? "
                "WHERE id=? AND status='collecting'",
                (last_message_id, int(time.time()), batch_id),
            )
            await db.commit()
            return cur.rowcount > 0
    ...
    async def cancel_message_batch(self, batch_id: int) -> bool:
        async with self.connection() as db:
            cur = await db.execute(
                "UPDATE custom_batches SET status='cancelled', completed_at=? "
                "WHERE id=? AND status='collecting'",
    ...
    async def set_batch_post_code(self, batch_id: int, code: str) -> None:
        async with self.connection() as db:
            await db.execute("UPDATE custom_batches SET post_code=? WHERE id=?", (code, batch_id))
            await db.commit()
    ...
    async def update_button(self, btn_id: int, **kwargs) -> bool:
        allowed = {"text", "url", "callback"}
        updates = {k: v for k, v in kwargs.items() if k in allowed}
        if not updates:
            return False
        async with self.connection() as db:
            set_clause = ", ".join(f"{k}=?" for k in updates)
            params = list(updates.values()) + [btn_id]
            cur = await db.execute(f"UPDATE button_configs SET {set_clause} WHERE id=?", params)
            await db.commit()
            return cur.rowcount > 0
    ...
    #
    # Single source of truth for the /start welcome flow. text / photo_id /
    # sticker_id / spoiler continue to be read from `settings` (that's
    # where they already lived, and where /setstartphoto, /setsticker,
    # /setwelcome and the settings: dashboard already read/write them —
    # changing the storage location would either lose existing production
    # data or force duplicating it). The extra animation/enable toggles the
    # new Welcome Customization UI needs live in the `welcome_config` row,
    # which used to be defined but never actually read anywhere.
    # ============================================================ #
    async def get_welcome_config(self) -> dict:
        ...
        return {
            ...
            "enabled": bool(int(extra.get("enabled", 1))),
            "anim_enabled": bool(int(extra.get("anim_enabled", 1))),
            "anim_type": extra.get("anim_type") or "line",
            "anim_speed": extra.get("anim_speed") or "normal",
            "sticker_anim_enabled": bool(int(extra.get("sticker_anim_enabled", 1))),
            "updated_at": extra.get("updated_at"),
        }

    async def update_welcome_config(self, **kwargs) -> dict:
        # Fields that are actually stored in `settings` (unchanged location).
        if "text" in kwargs:
        ...
        # Remaining fields (enabled/anim_enabled/anim_type/anim_speed/
        # sticker_anim_enabled) live in the welcome_config row.
        if kwargs:
            async with self.connection() as db:
                row = await (await db.execute("SELECT * FROM welcome_config WHERE id=1")).fetchone()
                current = dict(row) if row else {}
                merged = {
                    "enabled": int(current.get("enabled", 1)),
                    "anim_enabled": int(current.get("anim_enabled", 1)),
                    "anim_type": current.get("anim_type") or "line",
                    "anim_speed": current.get("anim_speed") or "normal",
                    "sticker_anim_enabled": int(current.get("sticker_anim_enabled", 1)),
                }
                for key in ("enabled", "anim_enabled", "sticker_anim_enabled"):
                    if key in kwargs:
                        merged[key] = 1 if kwargs[key] else 0
                if "anim_type" in kwargs:
                    merged["anim_type"] = kwargs["anim_type"]
                if "anim_speed" in kwargs:
                    merged["anim_speed"] = kwargs["anim_speed"]
                await db.execute(
                    "INSERT OR REPLACE INTO welcome_config"
                    "(id, text, photo_id, sticker_id, anim_enabled, anim_type, anim_speed, "
                    "updated_at, enabled, sticker_anim_enabled) "
                    "VALUES(1, '', NULL, NULL, ?, ?, ?, ?, ?, ?)",
                    (
                        merged["anim_enabled"], merged["anim_type"], merged["anim_speed"],
                        int(time.time()), merged["enabled"], merged["sticker_anim_enabled"],
                    )
                )
                await db.commit()
    ...
    async def reset_welcome_config(self) -> None:
        """Full reset — text/photo/sticker back to default (blank), all
        animation toggles back to their defaults, welcome re-enabled."""
        await self.set_setting("custom_welcome_html", "")
        await self.clear_start_photo()
        await self.clear_delivery_sticker()
        await self.set_setting("start_photo_spoiler", "1")
        async with self.connection() as db:
            await db.execute("DELETE FROM welcome_config WHERE id=1")
            await db.commit()
