...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        # Validate MongoDB configuration if selected
        db_backend = os.getenv("DB_BACKEND", "sqlite").strip().lower()
        if db_backend == "mongo":
            mongo_uri = os.getenv("MONGO_URI", "").strip()
            if not mongo_uri:
                raise RuntimeError(
                    "DB_BACKEND=mongo requires MONGO_URI environment variable. "
            ...
            if not mongo_uri.startswith("mongodb://") and not mongo_uri.startswith("mongodb+srv://"):
                raise RuntimeError(
                    f"MONGO_URI must start with 'mongodb://' or 'mongodb+srv://', got: {mongo_uri[:50]}..."
                )
