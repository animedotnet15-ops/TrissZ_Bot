"""Protected shortener guard, without Telegram-account binding, plus
health/status endpoints for deployment monitoring (Railway/Render/Docker)."""
...
async def health() -> JSONResponse:
    """Liveness probe for Railway/Render/Docker health checks — cheap,
    no external calls, just confirms the process + DB are responsive."""
    try:
    ...
    return JSONResponse({"status": "ok" if db_ok else "degraded", "database": db_ok}, status_code=status_code)
...
async def version() -> dict[str, str]:
    return {"bot": config.bot_username, "backend": config.db_backend}

@app.get("/status")
async def status() -> JSONResponse:
    """Deeper readiness probe: DB + basic counts. Deliberately does not
    make Telegram API calls here (that belongs in the bot-side /health
    command) so this endpoint stays fast and safe to poll frequently."""
    try:
        stats = await database.get_advanced_stats()
